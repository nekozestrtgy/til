...
```
スーパークラスのメソッドを使用する際は、上記の記述さえあれば特別な記法を用いる必要はない。通常通り、`（インスタンス名）.(スーパークラスのメソッド名)`で良い。
サブクラスでスーパークラスと同名のメソッドを定義すると、サブクラスのインスタンスで使用されるそのメソッドはサブクラスで定義された処理を行う＝オーバーライド
```
public void (オーバーライドするメソッド名) {
  super.(オーバーライドするメソッド名)();
  //付け足す処理
}
//こうするとオーバーライドする記述が楽になる。
```
...
//サブクラスのコンストラクタでは、必ずはじめにスーパークラスのコンストラクタを呼び出さなければならない。
```
...
```
サブクラスが共通に持つが処理内容がバラバラな場合、スーパークラスに詳細な処理内容を記述することができない。
その場合、スーパークラスを抽象クラスとし、抽象メソッドを定義する。
抽象クラスでは、インスタンスの生成ができないことに注意。


カプセル化
privateを使ったものは、カプセル化される。
基本的に、フィールドはprivateに、メソッドはpublicにしておくのがセオリー。
...
セッター
```
public void set(フィールド名)(仮引数) {
  this.(フィールド名) = (仮引数);
}
//これでクラス外からも安全にカプセル化されたフィールドの値を上書きできる。
```
...
```
private (クラス名) (変数名);
//これで、(クラス名)型の変数をフィールドとして定義することができる。

public (クラス名) get(クラス名)() {
  return this.(変数名);
}
//(クラス名)のインスタンスが返り値として渡される。

public void set(クラス名)（(クラス名) (変数名)） {
  this.(変数名) = (変数名);
}
```
